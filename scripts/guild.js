...
ServerBoard.prototype.init = async () => {
    ...
    document.getElementById('tags').innerHTML = `${guild.infomations.tags.map(v => `<a class="tag" href="./search.html?tags=${v}">${v}</a>`).join(' ')}`
    document.getElementById('lastUpdate').innerText = `${IsNotDate ? "最終更新 : 不明" : "最終更新 "+date.getMonth()+"/"+date.getDate()+" "+date.getHours()+":"+date.getMinutes()}`
